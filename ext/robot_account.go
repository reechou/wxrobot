...
const (
	...
	URI_ROBOT_ACCOUNT_ROBOT_GROUP  = "/robot/save_groups"
)
...
func (we *RobotAccount) RobotAddGroups(info *RobotSaveGroupsReq) error {
	u := "http://" + we.cfg.RobotAccount.Host + URI_ROBOT_ACCOUNT_ROBOT_GROUP
	logrus.Debugf("robot account add group: %v url: %s", info, u)
	...
	err = json.Unmarshal(rspBody, &response)
	if err != nil {
		logrus.Errorf("robot account add groups json decode error: %s", string(rspBody))
		return err
	}
	if response.Code != 0 {
		logrus.Errorf("robot account add groups error code[%d].", response.Code)
		return fmt.Errorf("robot account add groups error code[%d].", response.Code)
	}
